...
class LDAP
{
	...
	public function __construct($URI, $debug = false, $user, $pass)
	{
	    $this->_debug=$debug;
	...
	public function bind()
	{
        ...
        if ( ( ( $this->_username )
            && ( $this->_username != 'anonymous') )
            && ( $this->_password != '' ) ){
            $bind = ldap_bind($this->_ch, $this->_username."@".$this ->_server, $this->_password);
	...
	function search( $filter)
	{
        ...
        echo "<strong>DS:</strong> ".$this->_ch."<br>";
        echo "<strong>DN:</strong> ".$this->_baseDn."<br>";
        echo "<strong>FILTER:</strong> ".$filter."<br>";*/
        
		$result = ldap_search($this->_ch, $this->_baseDn, $filter);
	 ...
	 */
	public function authenticate( $username, $password, $filter='(samaccountname=%s)')
	{
		if($username!=''){
			//return true;
		    $this->connect();
		    $this->bind();
		    if(sprintf($filter, $username)!=''){
		    	
		    	$res = $this->search("samaccountname=".$username);	
		    }
