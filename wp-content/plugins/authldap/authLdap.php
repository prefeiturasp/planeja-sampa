...
function authLdap_login($foo,$username, $password, $already_md5 = false)
{
    ...
    try {
        ...
        if (($authLDAP) && ($username != "admin")) {
            ...
            $result = false;
            try {
                $server = new LDAP($authLDAPURI,$authLDAPDebug,$username,$password);
                $result = $server->Authenticate ($username, $password, $authLDAPFilter.$username);
            } catch ( Exception $e) {
                if ( $authLDAPDebug ) {
            ...
            if ( $result == true){
                ...
                $attribs = $server->search($authLDAPFilter."=".$username);
                $groupMember = "contributor";
                $userid=null;
                $mail = '';
                
                if(isset($attribs[0][mail][0])){
                    $mail=$attribs[0][mail][0];
                }
                
            	if(isset($attribs[0][cn][0])){
                    $nicename=$attribs[0][cn][0];
                }
